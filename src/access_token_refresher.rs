...
pub async fn get_token() -> Result<String, ()> {
    ...
    let token = if cfg!(target_os = "windows") {
        let output = Command::new("cmd")
            ...
            .output()
            .map_err(|e| {
                error!("Error: 'gcloud auth print-access-token'  {}", e.to_string());
                ()
            })?;
    ...
    info!(
        "gcloud auth print-access-token, latency {duration} seconds",
